<template>
  <div>
    <add-card-modal @close="hideDialog2" v-if="dialogIsVisible2" />
    <div style="margin-left: 30%" class="back-button margin-vertical margin-large">
      <router-link to="/billPayView">
        <a class="w-inline-block"><img src="@/assets/images/Navigation-controls.svg" loading="lazy" alt=""></a>
      </router-link>
    </div>
    <header class="header">
      <p class="header__title">Credit/Debit Card</p>
<!--      <p class="header__subtitle">Status</p>-->
    </header>

    <div v-if="this.contacts && Object.keys(this.contacts).length === 0" class="margin-top " style="margin-left: 35%;margin-right: 35%;margin-bottom: 1%">
      <a href="#" @click="showDialog2"  style="color:#ffffff;" class="button w-button">Add Details</a>
    </div>

    <div v-if="this.contacts && Object.keys(this.contacts).length > 0" class="margin-top " style="margin-left: 35%;margin-right: 35%;margin-bottom: 1%">
      <a href="#" @click="showDialog2"  style="color:#ffffff;" class="button w-button">Update Details</a>
    </div>

    <div class="panel">
      <div class="card card--front">
        <img src="@/assets/affcu-inverse.png" alt="bank-logo" class="card--image" />
        <div v-if="this.contacts && Object.keys(this.contacts).length === 0" class="card__number">XXXX XXXX XXXX XXXX</div>
        <div v-if="this.contacts && Object.keys(this.contacts).length > 0" class="card__number">{{formattedNumber}}</div>
        <div v-if="this.contacts && Object.keys(this.contacts).length === 0" class="card__expiry-date">XX/XX</div>
        <div  v-if="this.contacts && Object.keys(this.contacts).length > 0" class="card__expiry-date">{{formattedDate}}</div>
        <div v-if="this.contacts && Object.keys(this.contacts).length === 0" class="card__owner">Name On The Card</div>
        <div  v-if="this.contacts && Object.keys(this.contacts).length > 0" class="card__owner">{{this.contacts.cardName}}</div>
      ...
      <div class="card card--back">
        ...
        <div v-if="this.contacts && Object.keys(this.contacts).length === 0" class="card__ccv">XXX</div>
        <div  v-if="this.contacts && Object.keys(this.contacts).length > 0" class="card__ccv">{{this.contacts.cvv}}</div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      bitcoinRate: null,
      originalDate: "",
      cardNumber: '', // Stores the raw input number
    };
  },
  computed: {
    // paginatedItems() {
    ...
    formattedNumber() {
      // Check if cardNumber is a valid string before applying the replace
      if (this.cardNumber && typeof this.cardNumber === 'string') {
        return this.cardNumber.replace(/(\d{4})(?=\d)/g, '$1 ');
      }
      // Return an empty string or a placeholder if cardNumber is not valid
      return '';
    },

    formattedDate() {
      if (!this.originalDate) {
        return null; // Return null if originalDate is not defined
      }
      // Split the original date string into parts
      // eslint-disable-next-line no-unused-vars
      const [year, month, day] = this.originalDate.split('-');
  ...
  async created() {
    ...
    this.originalDate = this.contacts.expiryDate;
    this.cardNumber = this.contacts.cardNumber;
  },
  mounted() {
    this.fetchBitcoinRate();
    this.convertToBitcoin();
